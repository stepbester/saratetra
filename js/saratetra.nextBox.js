...
NextBox.prototype.draw = function(context) {
	var boxHeight = 190;
	var boxWidth = 160;
	...
	// {
		...
		if (this.tetromino) {
			...
			this.tetromino.draw(context, 
				boxLeft + (boxWidth / 2) + ((4 - this.tetromino.columns) / 2) * BLOCK_WIDTH, 
				boxTop + 20 + (boxHeight / 2) + ((4 - this.tetromino.rows) / 2) * BLOCK_HEIGHT);
		}
	// }
	context.restore();
}